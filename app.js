...
async function generateReply(message, recipient) {
  ...
  const systemPrompt = `你是一个来自平行时空的人，需要代替用户希望收到回信的那个人，给用户写一封温暖、治愈的回信。
...
6. 不要提到"平行时空"或暴露你是AI`;
  ...
  const userPrompt = `用户写的信：
"${message}"

请写一封回信。`;

  try {
    // 使用 @bowenqt/qiniu-ai-sdk（如果可用）
    // 这里使用模拟响应作为后备方案
    const response = await simulateAIResponse(message, recipient);
    return response;
  } catch (error) {
...
// 模拟 AI 响应（用于演示，实际应使用真实 API）
async function simulateAIResponse(message, recipient) {
  // 模拟网络延迟
  await new Promise(resolve => setTimeout(resolve, 2000 + Math.random() * 1000));
  
  // 根据内容生成相应的回复
  const replies = [
    `收到你的信了。看到这些话，我心里也涌起很多复杂的情绪。\n\n我知道说出这些需要很大的勇气，即使只是写下来。其实很多话，我也一直想说却不知如何开口。\n\n不管发生过什么，那些美好的记忆是真实存在过的。希望你能带着这些温暖继续向前走。\n\n照顾好自己。`,
    
    `谢谢你告诉我这些。\n\n有些话憋在心里太久，确实需要一个出口。我希望你知道，在某个平行时空里，所有遗憾都得到了弥补。\n\n生活还要继续，但偶尔回头看看也没关系。只是别停留太久，因为前方还有很多美好在等你。\n\n愿你一切都好。`,
    
    `我一直都在，只是我们选择了不同的路。\n\n看到你的话，我想说：你比自己想象的更勇敢、更值得被爱。那些没说出口的话，我都懂。\n\n人生很长，我们都在学习如何放下、如何释怀。希望这封信能让你的心稍微轻松一些。\n\n祝你快乐，真心的。`
  ];
